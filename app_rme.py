...
with st.sidebar:
    ...
    if not st.session_state.is_it_authenticated:
        ...
        menu = st.radio("Pilih Halaman:", menu_umum + menu_it)
...
# =========================================================
    ...
    db = init_db()
    df_arsip = pd.read_sql_query("SELECT * FROM rme_tasks WHERE status='Selesai'", db)
